...
class TPS_Morpher {
 public:
  ...
  /// Builds a matrix of (approximately) size N(p_samples) x
  /// N(p_samples*subsampling_factor),
  /// and inverts it using LU decomposition (O(N^3), so be careful not to input
  /// too large sample sets.
  ///
  /// For performance reasons, this function assumes ownership of the sample
  /// vector so don't
  /// change it after passing it here. Once you have done using the morpher, you
  /// can claim
  /// it back by calling grab_samples().
  ///
  /// The function assumes that the average distance between the points to be
  /// about 0.5 to
  /// save some computation. If this is not the case in your application, you
  /// need to calculate
  /// (or approximate) this value by yourself. See the comments concerning
  /// "double a" in the code.
  ///
  /// @param p_samples Displacement samples to be interpolated
  /// @param regularization Amount of "relaxation", 0.0 = exact interpolation
  /// @param subsampling_factor 1.0 = use all points, 0.5 = use 1/2 of the
  /// points etc.
  TPS_Morpher(std::auto_ptr<std::vector<Coord_Diff> > p_samples,
              double regularization, double subsampling_factor = 1.0)
      : samples(p_samples) {
    ...
    for (unsigned i = 0; i < p; ++i) {
      ...
      for (unsigned j = 0; j < m; ++j) {
        ...
        double elen2 =
            SQR(pt_i.x - pt_j.x) + SQR(pt_i.y - pt_j.y) + SQR(pt_i.z - pt_j.z);
    ...
    for (unsigned i = p; i < p + 4; ++i)
      for (unsigned j = m; j < m + 4; ++j) mtx_l(i, j) = 0.0;
  ...
  void morph(std::vector<Point>& pts) {
    ...
    for (std::vector<Point>::iterator ite = pts.begin(), end = pts.end();
      ...
      double dy = mtx_v(m + 0, 1) + mtx_v(m + 1, 1) * x + mtx_v(m + 2, 1) * y +
                  mtx_v(m + 3, 1) * z;
      double dz = mtx_v(m + 0, 2) + mtx_v(m + 1, 2) * x + mtx_v(m + 2, 2) * y +
                  mtx_v(m + 3, 2) * z;
      ...
      for (unsigned i = 0; i < m;
           ++i, ++diff_ite, ++cv0_ite, ++cv1_ite, ++cv2_ite) {
  ...
  double calc_bending_energy() {
    ...
    const unsigned m = mtx_orig_k.size2();
    ublas::matrix_range<Matrix> mtx_w(mtx_v, ublas::range(0, m),
                                      ublas::range(0, 3));
    ublas::matrix_range<Matrix> mtx_a(mtx_orig_k, ublas::range(0, m),
                                      ublas::range(0, m));
  ...
  std::auto_ptr<std::vector<Coord_Diff> > grab_samples() { return samples; }
