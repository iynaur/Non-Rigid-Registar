...
class Vec {
 public:
  ...
  Vec &operator*=(const float src) {
    x *= src;
    y *= src;
    z *= src;
    return *this;
  }
  Vec operator*(const float src) const {
    Vec tmp(*this);
    return (tmp *= src);
  }
  Vec &operator/=(const float src) {
...
inline Vec operator*(const float src, const Vec &v) {
  Vec tmp(v);
  return (tmp *= src);
}
...
inline Vec operator*(const Vec &v, const Mtx &m) {
  return Vec(m.data[0] * v.x + m.data[1] * v.y + m.data[2] * v.z + m.data[3],
