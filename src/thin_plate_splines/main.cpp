...
static void calc_tps() {
  ...
  for (unsigned i = 0; i < p; ++i) {
    for (unsigned j = i + 1; j < p; ++j) {
      ...
      mtx_l(i, j) = mtx_l(j, i) = mtx_orig_k(i, j) = mtx_orig_k(j, i) =
          tps_base_func(elen);
  ...
  for (unsigned i = 0; i < p; ++i) mtx_v(i, 0) = control_points[i].y;
  ...
  // Calc bending energy
  matrix<double> w(p, 1);
  for (int i = 0; i < p; ++i) w(i, 0) = mtx_v(i, 0);
...
static void clear_grid() {
  for (int x = 0; x < GRID_W; ++x)
    for (int z = 0; z < GRID_H; ++z) grid[x][z] = 0;
}
...
#define V_CROSS(A, B)                                     \
  ...
  if (mouseState[0] == mouseState[1] == mouseState[2] == 0) {
    ...
    glReadPixels(mouseX, view[3] - mouseY - 1, 1, 1, GL_DEPTH_COMPONENT,
                 GL_FLOAT, &z);
    gluUnProject(mouseX, view[3] - mouseY - 1, z, model, proj, view, &ox, &oy,
                 &oz);
  ...
  sprintf(tmp_str,
          "control points: %d, reqularization: %2.3f, bending energy: %4.3f",
...
static void mouse(int button, int state, int, int) {
  ...
  if (button == 1 && state == GLUT_DOWN) glutSetCursor(GLUT_CURSOR_CYCLE);
...
static void mouseMotion(int x, int y) {
  if (mouseState[0] && mouseX != -999)
    if (selected_cp >= 0) control_points[selected_cp].y += -(y - mouseY) / 3;
...
static void menu_select(int mode) { keyboard((unsigned char)mode, 0, 0); }
...
static void idlefunc() {
  glCheckErrors();
  if (screen_dirty) glutPostRedisplay();
}
