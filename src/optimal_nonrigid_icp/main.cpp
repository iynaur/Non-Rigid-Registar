...
int main(int argc, char** argv) {
  ...
  // vtkSmartPointer<vtkOBJReader> template_reader =
  // vtkSmartPointer<vtkOBJReader>::New();
  // template_reader->SetFileName(template_filename.c_str());
  // template_reader->Update();

  // vtkSmartPointer<vtkOBJReader> target_reader =
  // vtkSmartPointer<vtkOBJReader>::New();
  // target_reader->SetFileName(target_filename.c_str());
  // target_reader->Update();

  vtkSmartPointer<vtkPLYReader> template_reader =
      vtkSmartPointer<vtkPLYReader>::New();
  template_reader->SetFileName(template_filename.c_str());
  template_reader->Update();

  vtkSmartPointer<vtkPLYReader> target_reader =
      vtkSmartPointer<vtkPLYReader>::New();
  target_reader->SetFileName(target_filename.c_str());
  target_reader->Update();
  ...
  for (int i = 0; i < step; ++i) {
    ...
    char num_str[100];
    sprintf(num_str, "%d", i);
    plyWriter->SetFileName(
        (template_filename.substr(0, template_filename.find_last_of('.')) +
         "_" + num_str)
            .c_str());
    plyWriter->Write();
    alpha = alpha * 0.5;
  }
  vtkSmartPointer<vtkPolyDataMapper> template_mapper =
  ...
  vtkSmartPointer<vtkInteractorStyleRubberBandPick> interactorStyle =
